...
The popup can configure a custom upload URL, upload period, and device name, and displays the date and
time of the last successful upload. If either upload URL or upload period is left blank, the extension
uses the default value. The device name is initialized automatically on first install with a generated
...
5. Optionally enter a custom upload URL, upload period, or device name in the popup and click Save.
   Chrome asks for host permission when saving a custom upload URL.
